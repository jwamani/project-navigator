  ...
  return (
    <div>
      <section className="relative overflow-hidden bg-gradient-hero text-primary-foreground">
        <div className="absolute inset-0 opacity-10" aria-hidden style={{ backgroundImage: "radial-gradient(circle at 20% 20%, white 1px, transparent 1px), radial-gradient(circle at 80% 70%, white 1px, transparent 1px)", backgroundSize: "32px 32px" }} />
        <div className="relative mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8 lg:py-28">
          <div className="max-w-2xl">
            <p className="inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-3 py-1 text-xs font-medium uppercase tracking-wider">
              <BookOpen className="h-3 w-3" /> Online Library Management System
            </p>
            <h1 className="mt-5 text-4xl font-semibold leading-tight sm:text-5xl">
              A calm, modern home for your library catalogue.
            </h1>
            <p className="mt-5 max-w-xl text-base text-primary-foreground/80">
              Manage books, borrows, reservations, and members in one professional workspace built for libraries that value clarity over clutter.
            </p>
            <div className="mt-8 flex flex-wrap gap-3">
              <Button asChild size="lg" className="bg-accent text-accent-foreground hover:bg-accent/90">
                <Link to="/catalogue">Browse catalogue <ArrowRight className="ml-1 h-4 w-4" /></Link>
              </Button>
      ...
      <section className="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8">
        ...
        <div className="mt-12 grid gap-6 lg:grid-cols-3">
          {[
            ...
            { title: "Role-based access", body: "Admin, librarian, and member experiences tailored to each role's needs.", icon: ShieldCheck },
          ].map((f) => (
